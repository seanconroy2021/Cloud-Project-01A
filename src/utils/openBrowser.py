...
def wait_for_url(url, amount, sleep):
    ...
    while (
        True
    ):  # loop until we either get a response or time is greater than amount allowed.
        try:
        ...
        if (
            time.time() - start_time >= amount
        ):  # When time is greater than amount allowed return Fase meaning URL is not accessible.
